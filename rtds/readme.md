...
- app/server.py основной сервер
- api/server.py restapi
- storages/sqlite.py модуль сохранения данных в локальный буфер
- models/tag.py модель данных 
- configs/file.py модуль работы с конфигурацией
...
## Скрипты
Скрипты используются для выполнения промежуточных расчётов по считанным данным и заполнения новых тегов новыми значениями. 

Скрипты запускаются после выполнения операции чтения всеми коннекторами
...
возможности сбора этих данных сервером Historian. 
Сам RTDS не передаёт данные в Historian.
...
## Метрики
Для мониторинга сервиса предусмотреть сбор метрик:
- Среднее время цикла коннектора: сек
- Средняя скорость записи в архивный буфер: tag/сек 

Для передачи метрик сделать отдельную очередь публиковать метрики будет цетральный процесс.  
